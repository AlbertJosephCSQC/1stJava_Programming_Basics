...
next iteration. It can be accomplished by placing the keywords *`break`* or *`continue`*, respectively.
...
* After a break, all the remaining statements in the loop are skipped (terminated). The break statement can be used in a
  while loop, for loop, and do-while loop.

#### The `continue` Statement

* continue does not terminate the loop, it just skips some part of the loop.
* When we do not want to execute some statements then we use a continue statement to skip those statements from
  execution.
